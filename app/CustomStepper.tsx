...
const DEFAULT_QUEUE_STATE: QueueSocketState = {
  active: 0,
  waiting: 0,
  max_concurrent: 1,
  max_waiting: 3,
...
const mergeQueueSnapshot = (
  ...
  return {
    ...
    queue: queueSource
      ? {
          active: toNumberOr(queueSource.active, prev.active),
          waiting: toNumberOr(queueSource.waiting, prev.waiting),
          max_concurrent: toNumberOr(
            queueSource.max_concurrent,
            prev.max_concurrent,
          ),
          max_waiting: toNumberOr(queueSource.max_waiting, prev.max_waiting),
        }
      : prev.queue,
    analysis: {
      ...(prev.analysis || {}),
...
const CustomStepper = () => {
  ...
  const [q, setQ] = useState<QueueSocketState>(DEFAULT_QUEUE_STATE);
  ...
  useEffect(() => {
    ...
    s.on("queue_status", (payload) => {
      setQ((prev) => mergeQueueSnapshot(prev, payload));
    });
    ...
    s.on("analyze_progress", (payload) => {
      setQ((prev) => mergeAnalyzeProgress(prev, payload));
    });
